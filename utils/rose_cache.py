...
def parse_value_url(value_url_list: list):
    if not value_url_list:
        ...
        if not isinstance(value_url_list, list):
            value_url_list = [value_url_list]
        return [
...
def make_embed_with_info(info: dict):
    embed = discord.Embed(
        ...
        description="".join(parse_value_url(info.language)),
        url=info.title["url"],
    )
    embed.add_field(
        name="번호", value=f"[{info.galleryid}](https://hitomi.la/{info.galleryid}.html)"
    )
    embed.add_field(name="타입", value="".join(parse_value_url(info.type)))
