...
class Info(commands.Cog):
    ...
    async def _botinfo(self, ctx: commands.Context):
        ...
        embed.add_field(
            ...
            value=f"{f'{hiyobi_latency}ms' if hiyobi_latency else '가져올 수 없음'}",
        )
        embed.add_field(
            name="Average Heliotrope server latency",
            value=f"{f'{heliotrope_latency}ms' if heliotrope_latency else '가져올 수 없음'}",
        )
        embed.add_field(
            name="Average Pixiv API server latency",
            value=f"{f'{pixiv_latency}ms' if pixiv_latency else '가져올 수 없음'}",
        )
